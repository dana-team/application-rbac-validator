...
func TestBuildServerUrl(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			originalDomain := ServerUrlDomain
			ServerUrlDomain = tc.domain
			defer func() { ServerUrlDomain = originalDomain }()
...
func TestGetCurrentNamespace(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			originalPath := WebhookNamespacePath
		...
		},
		{
			...
			namespace: &corev1.Namespace{
				ObjectMeta: metav1.ObjectMeta{
					Name: "test-ns",
					Labels: map[string]string{
						AdminBypassLabel + "-my-cluster": "true",
					},
				},
			},
			clusterName: "my-cluster",
		...
		},
		{
			...
			namespace: &corev1.Namespace{
				ObjectMeta: metav1.ObjectMeta{
					Name: "test-ns",
					Labels: map[string]string{
						AdminBypassLabel + "-other-cluster": "true",
					},
				},
			},
			clusterName: "my-cluster",
...
func TestGetNamespacesList(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			result := GetNamespacesList(tc.secret)
...
func TestFetchArgoInstanceName(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			configMap: &corev1.ConfigMap{
				ObjectMeta: metav1.ObjectMeta{
					Name:      ArgoInstanceConfigMapName,
					Namespace: "test-namespace",
				},
				Data: map[string]string{
					ArgoInstanceNameConfigMapKey: "my-argo-instance",
				},
			},
			expectError: false,
	...
	testCases := []struct {
	...
	}{
		{
			...
			configMap: &corev1.ConfigMap{
				...
				Data: map[string]string{
					ArgoInstanceUsersConfigMapKey: "user1,user2,user3",
				},
			},
			expectError: false,
		...
		},
		{
			...
			configMap: &corev1.ConfigMap{
				...
				Data: map[string]string{
					ArgoInstanceUsersConfigMapKey: "user1",
				},
			},
			expectError: false,
...
func TestFetchClusterToken(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			configMap: &corev1.ConfigMap{
				ObjectMeta: metav1.ObjectMeta{
					Name:      ClusterTokensConfigMapName,
					Namespace: "test-namespace",
				},
				Data: map[string]string{
