...
It supports referencing destination clusters by both **Server URL** and **Cluster Name** (resolving the URL from Argo CD cluster secrets).
...
```sh
make docker-build docker-push IMG=<some-registry>/application-rbac-validator:tag
```
...
```sh
make deploy IMG=<some-registry>/application-rbac-validator:tag
```
...
```sh
make build-installer IMG=<some-registry>/application-rbac-validator:tag
```
...
```sh
kubectl apply -f https://raw.githubusercontent.com/<org>/application-rbac-validator/<tag or branch>/dist/install.yaml
```
